  ...
  async function importUsers(rows: Record<string, string>[]) {
    ...
    for (let index = 0; index < rows.length; index += 1) {
      try {
        ...
        if (!response.ok || !data.ok) throw new Error(data.message || "Không tạo được tài khoản");
        success += 1;
      } catch (error) {
        failed.push(`Dòng ${index + 1}: ${error instanceof Error ? error.message : "lỗi"}`);
      }
    }
    await load();
    if (failed.length) throw new Error(`Đã tạo ${success}/${rows.length}. ${failed.slice(0, 3).join(" · ")}`);
    setMessage(`Đã import ${success} tài khoản và lưu vào database.`);
    setShowForm(false);
  }
