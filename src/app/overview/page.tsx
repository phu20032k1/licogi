...
import { ArrowRight, Building2, CheckCircle2, Database, Factory, Globe2, MapPin, WalletCards } from "lucide-react";
...
export default function PublicOverviewPage() {
  ...
  const kpis = [
    { label: "Tổng công trình", value: projects.length, icon: Building2, href: "/projects" },
    { label: "Đã hoàn thành", value: completed.length, icon: CheckCircle2, href: "/projects?status=completed" },
    { label: "Đang thi công", value: ongoing.length, icon: Factory, href: "/projects?status=ongoing" },
    { label: "Tỉnh / thành", value: provinces, icon: MapPin, href: "/locations" },
    { label: "Lĩnh vực", value: sectors, icon: Factory, href: "/capabilities" },
    { label: "Quốc gia dự án", value: countries, icon: Globe2, href: "/locations" },
    { label: "Tiến độ bình quân", value: `${avgProgress}%`, icon: Database, href: "/projects?status=ongoing" },
    { label: "Độ đầy đủ dữ liệu", value: `${avgCompleteness}%`, icon: Database, href: "/projects" },
  ];
          ...
          {loading ? <div className="public-page-loading">Đang tổng hợp dữ liệu...</div> : error ? <div className="public-page-error">{error}</div> : <>
            ...
            <div className="public-overview-split">
              <article className="public-overview-status-panel">
                ...
                <Link href="/projects?status=completed"><span>Đã hoàn thành</span><strong>{completed.length}</strong><i style={{ width: `${projects.length ? completed.length / projects.length * 100 : 0}%` }}/></Link>
                <Link href="/projects?status=ongoing"><span>Đang thi công</span><strong>{ongoing.length}</strong><i style={{ width: `${projects.length ? ongoing.length / projects.length * 100 : 0}%` }}/></Link>
                <Link href="/projects?status=warranty"><span>Bảo hành</span><strong>{projects.filter((project) => project.status === "warranty").length}</strong><i style={{ width: `${projects.length ? projects.filter((project) => project.status === "warranty").length / projects.length * 100 : 0}%` }}/></Link>
              </article>

              <article className="public-overview-top-projects">
                <div className="public-page-block-head"><div><span>Danh mục theo giá trị</span><h2>Dự án nổi bật</h2></div><Link href="/projects">Xem tất cả <ArrowRight size={14}/></Link></div>
                <div className="public-overview-project-table">
                  {topProjects.map((project, index) => <Link key={project.id} href={`/projects/${encodeURIComponent(project.id)}`}><b>{String(index + 1).padStart(2, "0")}</b><span><strong>{project.name}</strong><small>{project.province} · {project.type}</small></span><em>{formatVnd(projectMoney(project))}</em><ArrowRight size={14}/></Link>)}
                </div>
              </article>
            </div>
          </>}
