...
export default function TasksPage() {
  ...
  async function submitTask(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      setForm(emptyForm);
      setShowForm(false);
      setMessage("Đã tạo công việc và lưu vào database. Refresh trang dữ liệu vẫn còn.");
      await load();
    } catch (err) {
  ...
  const actions = <button type="button" onClick={() => setShowForm(true)} className="inline-flex items-center gap-2 rounded-xl bg-orange-500 px-3.5 py-2.5 text-xs font-extrabold text-white shadow-md shadow-orange-200"><Plus size={16} /> Tạo công việc</button>;

  return <div className="space-y-5 animate-fade-up">
    <PageHeader eyebrow="Workflow Center" title="Công việc & phê duyệt" description="Công việc được lưu trực tiếp vào PostgreSQL/Prisma; có thể tạo từng việc hoặc import hàng loạt." actions={actions} />
    ...
    <section className="rounded-2xl border border-slate-200 bg-white p-3.5 shadow-sm">
      <div className="flex flex-col gap-3 lg:flex-row lg:items-center">
        <div className="flex w-fit rounded-xl border border-slate-200 bg-slate-50 p-1">{(["Công việc", "Phê duyệt"] as const).map((item) => <button key={item} type="button" onClick={() => { setTab(item); setStatus("all"); }} className={`rounded-lg px-3.5 py-2 text-xs font-extrabold ${tab === item ? "bg-slate-900 text-white shadow" : "text-slate-500"}`}>{item}</button>)}</div>
        <label className="flex min-w-0 flex-1 items-center gap-2 rounded-xl border border-slate-200 bg-slate-50 px-3 py-2.5 lg:ml-auto lg:max-w-md"><Search size={16} className="text-slate-400" /><input value={search} onChange={(event) => setSearch(event.target.value)} className="min-w-0 flex-1 bg-transparent text-sm outline-none" placeholder="Tìm nội dung, dự án, người phụ trách..." /></label>
        <label className="flex items-center gap-2 rounded-xl border border-slate-200 px-3 py-2.5"><Filter size={15} className="text-slate-400" /><select value={status} onChange={(event) => setStatus(event.target.value)} className="bg-transparent text-xs font-semibold text-slate-600 outline-none"><option value="all">Tất cả trạng thái</option>{tab === "Công việc" ? taskColumns.map((item) => <option key={item}>{item}</option>) : ["Chờ duyệt", "Đã duyệt", "Từ chối"].map((item) => <option key={item}>{item}</option>)}</select></label>
      </div>
    </section>
    ...
    {tab === "Công việc" ? <section className="grid gap-3 xl:grid-cols-4">{taskColumns.map((column) => {
      ...
      return <article key={column} className="rounded-2xl border border-slate-200 bg-slate-50/70 p-3"><div className="flex items-center justify-between px-1 py-1.5"><h2 className="text-xs font-black text-slate-800">{column}</h2><span className="rounded-full bg-white px-2 py-1 text-[10px] font-extrabold text-slate-500 ring-1 ring-slate-200">{columnTasks.length}</span></div><div className="mt-2 space-y-2.5">{columnTasks.map((task) => <div key={task.id} className="rounded-xl border border-slate-200 bg-white p-3.5 shadow-sm"><div className="flex items-start justify-between gap-2"><span className={`rounded-full px-2 py-1 text-[10px] font-extrabold ${task.priority === "Cao" ? "bg-red-50 text-red-700" : task.priority === "Trung bình" ? "bg-amber-50 text-amber-700" : "bg-sky-50 text-sky-700"}`}>{task.priority}</span></div><h3 className="mt-2.5 text-sm font-extrabold leading-5 text-slate-900">{task.title}</h3><p className="mt-1.5 text-xs text-slate-500">{task.project}</p><div className="mt-3"><div className="mb-1 flex justify-between text-[10px] font-bold text-slate-500"><span>Tiến độ</span><span>{task.progress}%</span></div><ProgressBar value={task.progress} tone={task.progress === 100 ? "green" : "orange"} /></div><div className="mt-3 flex items-center justify-between border-t border-slate-100 pt-2.5 text-[10px]"><span className="font-bold text-slate-600">{task.assignee}</span><span className="text-slate-400">{task.due ? `Hạn ${task.due}` : "Chưa có hạn"}</span></div></div>)}</div></article>;
    })}</section> : <section className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm"><div className="overflow-x-auto"><table className="w-full min-w-[900px] text-left text-xs"><thead className="bg-slate-50 text-[10px] font-extrabold uppercase tracking-[0.08em] text-slate-500"><tr><th className="px-4 py-3">Đề nghị</th><th className="px-4 py-3">Dự án</th><th className="px-4 py-3">Người đề nghị</th><th className="px-4 py-3">Trạng thái</th><th className="px-4 py-3 text-right">Xử lý</th></tr></thead><tbody className="divide-y divide-slate-100">{filteredApprovals.map((item) => <tr key={item.id}><td className="px-4 py-3 font-bold">{item.title}</td><td className="px-4 py-3">{item.project}</td><td className="px-4 py-3">{item.requester}</td><td className="px-4 py-3">{item.status}</td><td className="px-4 py-3"><div className="flex justify-end gap-2">{item.status === "Chờ duyệt" ? <><button type="button" onClick={() => updateApproval(item.id, "Từ chối")} className="rounded-lg border border-red-200 p-2 text-red-600"><XCircle size={15} /></button><button type="button" onClick={() => updateApproval(item.id, "Đã duyệt")} className="rounded-lg bg-emerald-500 p-2 text-white"><UserCheck size={15} /></button></> : null}</div></td></tr>)}</tbody></table></div>{!filteredApprovals.length ? <p className="p-6 text-center text-sm text-slate-500">Chưa có đề nghị phê duyệt.</p> : null}</section>}

    {showForm ? <div className="fixed inset-0 z-[80] flex items-end justify-center bg-slate-950/55 p-0 backdrop-blur-sm sm:items-center sm:p-5"><button className="absolute inset-0" onClick={() => setShowForm(false)} /><form onSubmit={submitTask} className="modal-panel relative w-full max-w-3xl rounded-t-[24px] bg-white p-5 shadow-2xl sm:rounded-[24px]"><div className="flex items-center justify-between"><div><p className="text-[10px] font-extrabold uppercase tracking-[0.14em] text-orange-600">New task</p><h2 className="mt-1 text-lg font-black text-slate-900">Tạo công việc mới</h2></div><button type="button" onClick={() => setShowForm(false)} className="rounded-xl border border-slate-200 px-3 py-2 text-xs font-bold">Đóng</button></div>
      ...
      <div className="mt-5 grid gap-3 md:grid-cols-2"><label className="text-xs font-bold text-slate-600 md:col-span-2">Tên công việc<input required value={form.title} onChange={(e)=>setForm({...form,title:e.target.value})} className="input-field mt-1.5 w-full rounded-xl px-3.5 py-2.5 text-sm" placeholder="Nhập tên công việc" /></label><label className="text-xs font-bold text-slate-600">Dự án<select value={form.projectId} onChange={(e)=>setForm({...form,projectId:e.target.value})} className="input-field mt-1.5 w-full rounded-xl px-3.5 py-2.5 text-sm"><option value="">Không gán</option>{projects.map((p)=><option key={p.id} value={p.id}>{p.code} · {p.name}</option>)}</select></label><label className="text-xs font-bold text-slate-600">Người phụ trách<select value={form.assigneeId} onChange={(e)=>setForm({...form,assigneeId:e.target.value})} className="input-field mt-1.5 w-full rounded-xl px-3.5 py-2.5 text-sm"><option value="">Chưa phân công</option>{users.map((u)=><option key={u.id} value={u.id}>{u.name} · {u.email}</option>)}</select></label><label className="text-xs font-bold text-slate-600">Hạn hoàn thành<input type="date" value={form.dueDate} onChange={(e)=>setForm({...form,dueDate:e.target.value})} className="input-field mt-1.5 w-full rounded-xl px-3.5 py-2.5 text-sm" /></label><label className="text-xs font-bold text-slate-600">Mức ưu tiên<select value={form.priority} onChange={(e)=>setForm({...form,priority:e.target.value as TaskItem["priority"]})} className="input-field mt-1.5 w-full rounded-xl px-3.5 py-2.5 text-sm"><option>Cao</option><option>Trung bình</option><option>Thấp</option></select></label></div><div className="mt-5 flex justify-end gap-3"><button type="button" onClick={() => setShowForm(false)} className="rounded-xl border border-slate-200 px-4 py-2.5 text-xs font-bold text-slate-600">Hủy</button><button disabled={saving} className="rounded-xl bg-orange-500 px-4 py-2.5 text-xs font-extrabold text-white disabled:bg-slate-300">{saving ? "Đang lưu..." : "Tạo công việc"}</button></div></form></div> : null}
  </div>;
}
