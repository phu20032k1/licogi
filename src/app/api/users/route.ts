...
type UserWriteBody = { id?: string; email?: string; name?: string; phone?: string; password?: string; roleCode?: string; departmentId?: string | null; customerId?: string | null; status?: string; mustChangePassword?: boolean };

function normalizeEmail(value?: string) { return String(value ?? "").trim().toLowerCase(); }
...
export async function GET() {
  ...
  const [users, roles, departments, customers] = await Promise.all([
    prisma.user.findMany({ where: { organizationId: auth.user.organizationId }, include: { role: true, department: true, customer: true }, orderBy: [{ status: "asc" }, { createdAt: "asc" }] }),
  ...
  return NextResponse.json({
    ok: true,
    users: users.map((user: UserListRow) => ({ id: user.id, email: user.email, name: user.name, phone: user.phone ?? "", status: user.status, mustChangePassword: user.mustChangePassword, roleId: user.roleId, role: user.role.name, roleCode: user.role.code, dataScope: user.role.dataScope, departmentId: user.departmentId, department: user.department?.name ?? "", customerId: user.customerId, customer: user.customer?.name ?? "", lastLoginAt: user.lastLoginAt?.toISOString() ?? "", createdAt: user.createdAt.toISOString() })),
...
export async function POST(request: Request) {
  ...
  if (!email || !body?.name || !body?.password || !body?.roleCode) return NextResponse.json({ ok: false, message: "Thiếu tên, email, mật khẩu hoặc vai trò." }, { status: 400 });
  ...
  const user = await prisma.user.create({ data: { organizationId: auth.user.organizationId, email, name: body.name.trim(), phone: body.phone?.trim() || null, passwordHash: createPasswordHash(body.password), roleId: role.id, departmentId: body.departmentId || null, customerId: body.customerId || null, status: safeStatus(body.status), mustChangePassword: false } });
...
export async function PATCH(request: Request) {
  ...
  if (body.id === auth.user.id && body.status === AccountStatus.SUSPENDED) return NextResponse.json({ ok: false, message: "Không thể tự khóa tài khoản đang đăng nhập." }, { status: 400 });
...
export async function DELETE(request: Request) {
  ...
  if (!ids.length) return NextResponse.json({ ok: false, message: "Chưa chọn tài khoản cần xóa." }, { status: 400 });
  if (ids.includes(auth.user.id)) return NextResponse.json({ ok: false, message: "Không thể xóa tài khoản đang đăng nhập." }, { status: 400 });
  if (ids.length > 200) return NextResponse.json({ ok: false, message: "Mỗi lần chỉ xóa tối đa 200 tài khoản." }, { status: 413 });
  const users = await prisma.user.findMany({ where: { organizationId: auth.user.organizationId, id: { in: ids } }, select: { id: true, email: true } });
  if (!users.length) return NextResponse.json({ ok: false, message: "Không tìm thấy tài khoản hợp lệ." }, { status: 404 });
  const validIds = users.map((user) => user.id);
  await prisma.user.deleteMany({ where: { organizationId: auth.user.organizationId, id: { in: validIds } } });
  await audit(auth.user, ModuleCode.USERS, PermissionAction.DELETE, `Xóa ${validIds.length} tài khoản.`, "user", validIds[0], { ids: validIds, emails: users.map((user) => user.email) });
