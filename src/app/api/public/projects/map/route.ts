...
const MAP_CACHE = "public, max-age=120, s-maxage=300, stale-while-revalidate=1800";
  ...
  try {
    ...
    if (!organization) {
      return NextResponse.json(
        { ok: true, total: 0, projects: [], generatedAt: new Date().toISOString(), view: "map" },
        { headers: { "Cache-Control": MAP_CACHE } },
      );
    }
