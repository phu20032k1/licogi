...
import { CheckCircle2, Clock3, Plus, Search, ShieldCheck, Siren, Trash2, Wrench } from "lucide-react";
...
type WarrantyTicket = { id: string; code: string; projectCode: string; project: string; title: string; customer: string; priority: string; created: string; deadline: string; status: string };
...
export default function WarrantyPage() {
  ...
  const allFilteredSelected = Boolean(filtered.length) && filtered.every((ticket) => selectedSet.has(ticket.id));
  ...
  async function importRows(rows: Record<string, string>[]) {
    const normalized = rows.map((row, index) => ({ ...row, ticket_code: row.ticket_code || `BH-${Date.now()}-${String(index + 1).padStart(3, "0")}`, status: row.status || "Đã tiếp nhận" }));
  ...
  async function createTicket(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      await importRows([{ ticket_code: `BH-${Date.now()}`, project_code: form.projectCode, title: form.title, priority: form.priority, deadline: form.deadline, status: "Đã tiếp nhận", description: form.description }]);
      setForm(emptyForm);
      setMessage("Đã tạo yêu cầu bảo hành và lưu vào database.");
    } catch (err) {
      setError(err instanceof Error ? err.message : "Không lưu được yêu cầu bảo hành.");
    } finally { setSaving(false); }
  }

  function toggleTicket(id: string) { setSelectedIds((current) => current.includes(id) ? current.filter((item) => item !== id) : [...current, id]); }
  ...
  async function deleteTickets(ids: string[]) {
    ...
    if (!window.confirm(`Xóa ${ids.length} yêu cầu bảo hành đã chọn?`)) return;
    setDeleting(true); setError("");
    try {
      const response = await fetch("/api/data/warranty", { method: "DELETE", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ ids }) });
      const data = await response.json().catch(() => ({}));
      if (!response.ok || !data.ok) throw new Error(data.message || "Không xóa được yêu cầu bảo hành.");
      setMessage(`Đã xóa ${ids.length} yêu cầu bảo hành.`);
      await load();
    } catch (err) {
      setError(err instanceof Error ? err.message : "Không xóa được yêu cầu bảo hành.");
    } finally { setDeleting(false); }
  }

  return <div className="space-y-5 animate-fade-up">
    <PageHeader eyebrow="Warranty Service" title="Quản lý bảo hành công trình" description="Theo dõi yêu cầu, SLA và xử lý hàng loạt trên dữ liệu PostgreSQL." actions={<button onClick={() => setShowForm(true)} className="licogi-btn licogi-btn-primary"><Plus size={16} /> Tạo yêu cầu</button>} />
    ...
    <section className="grid gap-5 xl:grid-cols-[1fr_290px]">
      <div className="space-y-3 min-w-0">
        <div className="rounded-[18px] border border-slate-200 bg-white p-3.5 shadow-sm">
          <div className="grid gap-3 md:grid-cols-[1fr_200px_auto]"><label className="flex items-center gap-2 rounded-[12px] border border-slate-200 bg-slate-50 px-3 py-2.5"><Search size={16} className="text-slate-400" /><input value={search} onChange={(event) => setSearch(event.target.value)} className="min-w-0 flex-1 bg-transparent text-sm outline-none" placeholder="Tìm mã, dự án, nội dung yêu cầu..." /></label><select value={status} onChange={(event)=>setStatus(event.target.value)} className="rounded-[12px] border border-slate-200 px-3 py-2.5 text-xs font-semibold text-slate-600"><option value="all">Tất cả trạng thái</option><option>Đã tiếp nhận</option><option>Đang xử lý</option><option>Hoàn thành</option></select><button type="button" onClick={toggleAllFiltered} className="licogi-btn licogi-btn-secondary">{allFilteredSelected ? "Bỏ chọn" : "Chọn tất cả"}</button></div>
          {selectedIds.length ? <div className="mt-3 flex items-center justify-between gap-3 border-t border-slate-100 pt-3"><p className="text-xs font-bold text-slate-600">Đã chọn {selectedIds.length}</p><button type="button" onClick={() => void deleteTickets(selectedIds)} disabled={deleting} className="licogi-btn licogi-btn-danger"><Trash2 size={15} /> {deleting ? "Đang xóa..." : `Xóa ${selectedIds.length}`}</button></div> : null}
        </div>

        <div className="overflow-hidden rounded-[18px] border border-slate-200 bg-white shadow-sm">
          <div className="licogi-table-scroll max-h-[680px] overflow-auto"><table className="w-full min-w-[960px] text-left text-xs"><thead className="sticky top-0 z-10 bg-slate-50/95 text-[10px] font-extrabold uppercase tracking-[0.08em] text-slate-500 backdrop-blur"><tr><th className="w-12 px-4 py-3"><input aria-label="Chọn tất cả yêu cầu đang hiển thị" type="checkbox" checked={allFilteredSelected} onChange={toggleAllFiltered} className="h-4 w-4 rounded border-slate-300 accent-orange-600" /></th><th className="px-4 py-3">Yêu cầu</th><th className="px-4 py-3">Công trình</th><th className="px-4 py-3">Khách hàng</th><th className="px-4 py-3">Ưu tiên</th><th className="px-4 py-3">SLA</th><th className="px-4 py-3">Trạng thái</th><th className="sticky right-0 bg-slate-50/95 px-4 py-3 text-right">Xóa</th></tr></thead><tbody className="divide-y divide-slate-100">{filtered.map((ticket)=>{
            const selected = selectedSet.has(ticket.id);
            return <tr key={ticket.id} className={selected ? "bg-orange-50/60" : "hover:bg-slate-50/70"}><td className="px-4 py-3"><input aria-label={`Chọn ${ticket.code}`} type="checkbox" checked={selected} onChange={() => toggleTicket(ticket.id)} className="h-4 w-4 rounded border-slate-300 accent-orange-600" /></td><td className="px-4 py-3"><p className="max-w-[260px] truncate font-extrabold text-slate-900">{ticket.title}</p><p className="mt-1 text-[10px] font-semibold text-slate-400">{ticket.code} · {ticket.created}</p></td><td className="max-w-[200px] px-4 py-3"><p className="truncate font-semibold text-slate-600">{ticket.project}</p></td><td className="max-w-[180px] px-4 py-3"><p className="truncate text-slate-600">{ticket.customer}</p></td><td className="px-4 py-3"><span className={`rounded-[9px] px-2 py-1 text-[10px] font-extrabold ${ticket.priority === "Cao" ? "bg-red-50 text-red-700" : ticket.priority === "Trung bình" ? "bg-amber-50 text-amber-700" : "bg-sky-50 text-sky-700"}`}>{ticket.priority}</span></td><td className="px-4 py-3 font-bold text-slate-700">{ticket.deadline}</td><td className="px-4 py-3"><span className={`rounded-[9px] px-2 py-1 text-[10px] font-extrabold ${ticket.status === "Hoàn thành" ? "bg-emerald-50 text-emerald-700" : ticket.status === "Đang xử lý" ? "bg-orange-50 text-orange-700" : "bg-sky-50 text-sky-700"}`}>{ticket.status}</span></td><td className={`sticky right-0 px-4 py-3 text-right ${selected ? "bg-orange-50" : "bg-white"}`}><button type="button" onClick={() => void deleteTickets([ticket.id])} className="licogi-icon-btn licogi-icon-btn-danger"><Trash2 size={14} /></button></td></tr>;
          })}</tbody></table></div>{filtered.length === 0 ? <p className="p-8 text-center text-sm text-slate-500">Chưa có yêu cầu bảo hành.</p> : null}</div>
      </div>
      <aside className="space-y-4"><article className="rounded-[18px] bg-[#071426] p-5 text-white shadow-lg"><ShieldCheck size={23} className="text-orange-400" /><p className="mt-4 text-xs font-bold text-slate-300">Tỷ lệ hoàn thành</p><p className="mt-1 text-3xl font-black">{slaRate}%</p><div className="mt-3 h-2 rounded-full bg-white/10"><div className="h-full rounded-full bg-gradient-to-r from-orange-400 to-orange-600" style={{width:`${slaRate}%`}} /></div></article><article className="rounded-[18px] border border-slate-200 bg-white p-4 shadow-sm"><h3 className="text-sm font-black text-slate-900">SLA theo nhóm</h3><div className="mt-4 space-y-3">{["Xây dựng", "MEP", "Hoàn thiện", "Khác"].map((label)=><div key={label}><div className="mb-1 flex justify-between text-[10px]"><span className="font-bold text-slate-600">{label}</span><span className="font-black text-slate-800">0%</span></div><ProgressBar value={0} tone="slate" /></div>)}</div></article></aside>
    </section>

    {showForm ? <div className="fixed inset-0 z-[80] flex items-end justify-center bg-slate-950/55 p-0 backdrop-blur-sm sm:items-center sm:p-5"><button className="absolute inset-0" onClick={()=>setShowForm(false)} aria-label="Đóng" /><form onSubmit={createTicket} className="modal-panel licogi-scroll relative max-h-[90vh] w-full max-w-3xl overflow-y-auto rounded-t-[22px] bg-white p-5 shadow-2xl sm:rounded-[20px]"><div className="flex items-center justify-between"><div><p className="text-[10px] font-extrabold uppercase tracking-[0.14em] text-orange-600">Warranty ticket</p><h2 className="mt-1 text-lg font-black text-slate-900">Tạo yêu cầu bảo hành</h2></div><button type="button" onClick={()=>setShowForm(false)} className="licogi-btn licogi-btn-secondary">Đóng</button></div>
      ...
      <div className="mt-5 grid gap-3 md:grid-cols-2"><label className="text-xs font-bold text-slate-600 md:col-span-2">Công trình<select value={form.projectCode} onChange={(e)=>setForm({...form,projectCode:e.target.value})} className="input-field mt-1.5 w-full rounded-[12px] px-3.5 py-2.5 text-sm"><option value="">Không gán dự án</option>{projects.map((project)=><option key={project._id} value={project.project_code || ""}>{project.project_code} · {project.project_name}</option>)}</select></label><label className="text-xs font-bold text-slate-600 md:col-span-2">Tiêu đề<input required value={form.title} onChange={(e)=>setForm({...form,title:e.target.value})} className="input-field mt-1.5 w-full rounded-[12px] px-3.5 py-2.5 text-sm" placeholder="Nội dung cần bảo hành" /></label><label className="text-xs font-bold text-slate-600">Mức ưu tiên<select value={form.priority} onChange={(e)=>setForm({...form,priority:e.target.value})} className="input-field mt-1.5 w-full rounded-[12px] px-3.5 py-2.5 text-sm"><option>Thấp</option><option>Trung bình</option><option>Cao</option></select></label><label className="text-xs font-bold text-slate-600">Hạn xử lý<input type="date" value={form.deadline} onChange={(e)=>setForm({...form,deadline:e.target.value})} className="input-field mt-1.5 w-full rounded-[12px] px-3.5 py-2.5 text-sm" /></label><label className="text-xs font-bold text-slate-600 md:col-span-2">Mô tả<textarea value={form.description} onChange={(e)=>setForm({...form,description:e.target.value})} className="input-field mt-1.5 min-h-24 w-full rounded-[12px] px-3.5 py-2.5 text-sm" /></label></div><div className="mt-5 flex justify-end gap-3"><button type="button" onClick={()=>setShowForm(false)} className="licogi-btn licogi-btn-secondary">Hủy</button><button disabled={saving} className="licogi-btn licogi-btn-primary">{saving ? "Đang lưu..." : "Tạo yêu cầu"}</button></div></form></div> : null}
  </div>;
}
