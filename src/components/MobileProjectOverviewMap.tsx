...
export default function MobileProjectOverviewMap({ projects, loading = false, error = "", onReload, compact = false }: Props) {
  ...
  return <section className={`phone-data-map ${compact ? "is-compact" : ""}`} aria-label="Bản đồ công trình LICOGI 18.3">
    <div className="phone-data-map-head">
      <div>
        <span>BẢN ĐỒ CÔNG TRÌNH</span>
        <strong>{projects.length > 0 ? `${projects.length} dự án · ${provinces.size} tỉnh/thành` : loading ? "Danh mục đang được cập nhật" : "Bản đồ tạm thời chưa khả dụng"}</strong>
      </div>
    ...
    <div className="phone-data-map-canvas">
      ...
      {projects.length === 0 && !loading ? <div className="phone-data-map-state">
        ...
        {error ? <span>{error}</span> : <span>Hệ thống chưa nhận được danh mục công trình.</span>}
        {onReload ? <button type="button" onClick={onReload}><RefreshCcw size={15}/> Cập nhật lại</button> : null}
      </div> : null}
      ...
      <div className="phone-data-map-legend" aria-label="Chú thích trạng thái">
        ...
        <span><i className="is-warranty"/>Bảo hành <b>{warranty}</b></span>
      </div>
    </div>

    <Link href="/portfolio/projects" className="phone-data-map-action"><MapPin size={16}/> Xem toàn bộ danh mục công trình <ArrowRight size={15}/></Link>
  </section>;
}
