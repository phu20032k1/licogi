      ...
      <section className="public-page-section public-project-detail-content">
        <div className="public-container">
          <div className="public-project-detail-finance">
            <article><WalletCards/><span>Giá trị hợp đồng</span><strong>{formatVnd(financial.contractValueVnd || project.contractValueVnd, "Chưa ghi nhận")}</strong></article>
            <article><FileText/><span>Đề nghị thanh toán</span><strong>{formatVnd(financial.paymentRequestedVnd, "Chưa ghi nhận")}</strong></article>
            <article><ShieldCheck/><span>Đã thanh toán</span><strong>{formatVnd(financial.paymentPaidVnd, "Chưa ghi nhận")}</strong></article>
            <article><Database/><span>Phải thu còn lại</span><strong>{formatVnd(financial.outstandingReceivableVnd, "Chưa ghi nhận")}</strong></article>
          </div>
          ...
          <div className="public-project-detail-layout">
            <div className="public-project-detail-main">
              <article className="public-detail-panel public-detail-map-panel">
                <div className="public-detail-panel-head">
                  <div><MapPin/><span>Vị trí dự án</span></div>
              ...
              <article className="public-detail-panel">
                ...
                <div className="public-project-detail-rows">
                  ...
                  <DetailRow label="Mã khách hàng" value={project.customerCode}/>
                  <DetailRow label="Ngành chủ đầu tư" value={project.customerIndustry}/>
                  ...
                  <DetailRow label="Số bản ghi hợp đồng" value={financial.contractCount}/>
                </div>
              </article>
              ...
              <article className="public-detail-panel">
                ...
                <div className="public-project-detail-rows">
                  ...
                  <DetailRow label="Tọa độ" value={hasCoordinates ? `${project.lat.toFixed(5)}, ${project.lng.toFixed(5)}` : "Đang cập nhật"}/>
                </div>
              </article>
            ...
            <aside className="public-project-detail-aside">
              <article className="public-detail-panel public-detail-progress-panel">
                ...
                <div className="public-detail-progress-number"><strong>{project.progress}%</strong><span>tiến độ</span></div>
                ...
                <div className="public-project-detail-rows">
                  ...
                  <DetailRow label="Chỉ số vận hành" value={project.healthScore !== undefined ? `${project.healthScore}/100` : undefined}/>
                  <DetailRow label="Độ đầy đủ dữ liệu" value={project.dataCompleteness !== undefined ? `${project.dataCompleteness}%` : undefined}/>
                  <DetailRow label="Mức rủi ro" value={project.risk}/>
                  <DetailRow label="Khởi công" value={project.startDate ? formatPublicDate(project.startDate) : undefined}/>
                  <DetailRow label="Kết thúc" value={project.endDate ? formatPublicDate(project.endDate) : undefined}/>
                  <DetailRow label="Nguồn dữ liệu" value={project.source}/>
                  <DetailRow label="Cập nhật" value={formatPublicDate(project.updatedAt)}/>
                </div>
              </article>
              ...
              <article className="public-detail-panel">
                ...
                <div className="public-detail-related-grid">
                  <div><strong>{related.contracts || 0}</strong><span>Hợp đồng</span></div>
                  <div><strong>{related.paymentRequests || 0}</strong><span>Thanh toán</span></div>
                  <div><strong>{related.debtLedgers || 0}</strong><span>Công nợ</span></div>
                  <div><strong>{related.documents || 0}</strong><span>Hồ sơ</span></div>
                  <div><strong>{related.equipment || 0}</strong><span>Thiết bị</span></div>
                  <div><strong>{related.tasks || 0}</strong><span>Công việc</span></div>
                  <div><strong>{related.dailyReports || 0}</strong><span>Nhật ký</span></div>
                  <div><strong>{related.bimModels || 0}</strong><span>BIM</span></div>
                  <div><strong>{related.warranties || 0}</strong><span>Bảo hành</span></div>
                </div>
              </article>
