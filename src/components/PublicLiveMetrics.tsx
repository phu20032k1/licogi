...
export default function PublicLiveMetrics() {
  ...
  const metrics = [
    { label: "Tổng công trình", value: loading ? "—" : String(projects.length), href: "/portfolio/projects", icon: Building2 },
    { label: "Đã hoàn thành", value: loading ? "—" : String(completed.length), href: "/portfolio/projects?status=completed", icon: CheckCircle2 },
    { label: "Đang thi công", value: loading ? "—" : String(ongoing.length), href: "/portfolio/projects?status=ongoing", icon: Factory },
    { label: "Tổng giá trị hợp đồng", value: loading ? "—" : formatVnd(contractValue, "0 đồng"), href: "/portfolio/overview", icon: WalletCards, wide: true },
    { label: "Tỉnh / thành", value: loading ? "—" : String(provinces), href: "/portfolio/locations", icon: MapPin },
    { label: "Lĩnh vực", value: loading ? "—" : String(sectors), href: "/portfolio/capabilities", icon: Factory },
  ];
  ...
  return <section className="public-live-overview public-live-overview-simple" aria-label="Tổng quan dữ liệu năng lực">
    <div className="public-live-overview-head">
      ...
      <Link href="/portfolio/overview">Xem chi tiết <ArrowUpRight size={14}/></Link>
    </div>
