...
export default function ProjectMap({ compact = false }: Props) {
  ...
  return (
    <div className={`grid gap-5 ${compact ? "2xl:grid-cols-[310px_1fr]" : "xl:grid-cols-[360px_1fr]"}`}>
      <aside className="order-2 overflow-hidden rounded-[24px] border border-slate-200 bg-white shadow-sm xl:order-1">
        <div className="border-b border-slate-100 p-4">
          <div className="flex items-center gap-2">
            <label className="flex min-w-0 flex-1 items-center gap-2 rounded-xl border border-slate-200 bg-slate-50 px-3 py-2.5 focus-within:border-orange-300 focus-within:ring-4 focus-within:ring-orange-50">
              <Search size={16} className="text-slate-400" />
              <input value={search} onChange={(event) => setSearch(event.target.value)} className="min-w-0 flex-1 bg-transparent text-sm outline-none" placeholder="Tìm dự án hoặc địa điểm" />
            </label>
            <button type="button" onClick={() => void sync(false)} className="rounded-xl border border-slate-200 p-2.5 text-slate-500 transition hover:border-orange-200 hover:text-orange-600" aria-label="Tải lại">
              <RefreshCcw size={16} className={loading ? "animate-spin" : ""}/>
            </button>
          </div>
          <div className="mt-2 grid grid-cols-2 gap-2">
            <select aria-label="Lọc loại dự án" value={type} onChange={(event) => setType(event.target.value as "all" | ProjectType)} className="min-w-0 rounded-xl border border-slate-200 bg-white px-2.5 py-2 text-xs font-semibold text-slate-600 outline-none">
              <option value="all">Tất cả loại</option>
              {projectTypes.map((item) => <option key={item} value={item}>{item}</option>)}
            </select>
            <select aria-label="Lọc trạng thái dự án" value={status} onChange={(event) => setStatus(event.target.value as "all" | ProjectStatus)} className="min-w-0 rounded-xl border border-slate-200 bg-white px-2.5 py-2 text-xs font-semibold text-slate-600 outline-none">
        ...
        <div className="grid grid-cols-3 gap-2 border-b border-slate-100 p-3 text-center text-xs">
          <div className="rounded-xl bg-slate-50 p-2.5"><p className="text-lg font-black text-slate-950">{validProjects.length}</p><p className="text-[10px] text-slate-500">Dự án</p></div>
          <div className="rounded-xl bg-slate-50 p-2.5"><p className="text-lg font-black text-slate-950">{fdiCount}</p><p className="text-[10px] text-slate-500">FDI</p></div>
          <div className="rounded-xl bg-slate-50 p-2.5"><p className="text-lg font-black text-slate-950">{new Set(validProjects.map((project) => project.province)).size}</p><p className="text-[10px] text-slate-500">Tỉnh</p></div>
        </div>

        <div className={`${compact ? "max-h-[370px]" : "max-h-[610px]"} divide-y divide-slate-100 overflow-y-auto`}>
          {validProjects.map((project) => (
            <button key={project.id} type="button" onClick={() => setSelectedId(project.id)} className={`w-full p-4 text-left transition hover:bg-slate-50 ${selected?.id === project.id ? "bg-orange-50/70" : "bg-white"}`}>
              <div className="flex items-start justify-between gap-2">
                <div className="min-w-0"><p className="truncate text-sm font-extrabold text-slate-900">{project.name}</p><p className="mt-1 flex items-center gap-1 truncate text-[11px] text-slate-500"><MapPin size={12} className="text-orange-500" /> {project.province} · {project.type}</p></div>
      ...
      <section className="order-1 min-w-0 xl:order-2">
        <div className="overflow-hidden rounded-[24px] border border-slate-200 bg-white p-2 shadow-sm">
          <MapContainer center={[16.2, 106]} zoom={5} minZoom={4} scrollWheelZoom className={`${mapHeight} w-full rounded-2xl`}>
            ...
            {validProjects.map((project) => {
              ...
              return <Marker
                ...
                icon={L.divIcon({
                  ...
                  iconSize: isSelected ? [50, 58] : [44, 52],
                  iconAnchor: isSelected ? [25, 56] : [22, 50],
                  popupAnchor: [0, -45],
                })}
                eventHandlers={{ click: () => setSelectedId(project.id) }}
              >
                <Tooltip direction="top" offset={[0, -42]} opacity={0.95}>{project.name}</Tooltip>
                <Popup>
        ...
        {selected ? <div className="mt-3 rounded-[20px] border border-slate-200 bg-white p-4 shadow-sm">
          <div className="flex flex-wrap items-center gap-2"><span className="rounded-lg bg-orange-50 px-2 py-1 text-[10px] font-black uppercase tracking-[0.12em] text-orange-700">{selected.code}</span><StatusBadge status={selected.status} /></div>
          <h3 className="mt-2 text-base font-black text-slate-950">{selected.name}</h3>
          <p className="mt-1 text-sm text-slate-500">{selected.province} · {selected.investor}</p>
        </div> : null}
      </section>
    </div>
  );
