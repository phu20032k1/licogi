...
type EmployeeDbRow = DbBaseRow & { code: string; fullName: string; englishName?: string | null; skillGroup?: string | null; position?: string | null; quantity?: number | null; phone?: string | null; assignedProjectCode?: string | null };
type EquipmentDbRow = DbBaseRow & { code: string; name: string; category: string; quantity?: string | null; quantityNumber?: number | null; specifications?: string | null; project?: { code?: string | null } | null; status?: string | null; maintenanceDate?: Date | null };
type DocumentDbRow = DbBaseRow & { code: string; name: string; project?: { code?: string | null } | null; type?: string | null; revision?: string | null; status?: string | null; fileUrl?: string | null };
...
export async function readRows(entity: DataEntityKey, context?: StoreContext): Promise<StoredRow[]> {
  ...
  if (entity === "employees") {
    ...
    return rows.map((row: EmployeeDbRow) => ({ ...base(row), employee_code: row.code, full_name: row.fullName, english_name: s(row.englishName), department: s(row.skillGroup || "Phòng Nhân sự"), position: s(row.position), quantity: s(row.quantity), phone: s(row.phone), assigned_project: s(row.assignedProjectCode) }));
  }
  if (entity === "equipment") {
    const rows = await prisma.equipment.findMany({ where: equipmentScope(context?.user), include: { project: true }, orderBy: { updatedAt: "desc" } });
    return rows.map((row: EquipmentDbRow) => ({ ...base(row), equipment_code: row.code, equipment_name: row.name, category: row.category, quantity: s(row.quantity), quantity_number: s(row.quantityNumber), specifications: s(row.specifications), project_code: s(row.project?.code), status: s(row.status), maintenance_date: row.maintenanceDate ? row.maintenanceDate.toISOString().slice(0, 10) : "" }));
  }
  if (entity === "documents") {
    const rows = await prisma.document.findMany({ where: documentScope(context?.user), include: { project: true }, orderBy: { updatedAt: "desc" } });
    return rows.map((row: DocumentDbRow) => ({ ...base(row), document_code: row.code, document_name: row.name, project_code: s(row.project?.code), document_type: s(row.type), revision: s(row.revision), status: s(row.status), file_url: s(row.fileUrl) }));
  }
